...
export default function EWalletCheckoutPage() {
  ...
  const handleWalletSelect = async (
    ...
    try {
      // Step 1: Create payment method; default expiry is 2 minutes for live, no expiry for test
      const expirySeconds =
        process.env.NEXT_PUBLIC_PAYMONGO_LIVE === 'true' ? 120 : undefined;
      // only qrph supports for live environment, gcash and paymaya will use test mode with no expiry
      const walletTypeForApi =
        process.env.NEXT_PUBLIC_PAYMONGO_LIVE === 'true' &&
        walletType === 'qrph'
          ? 'qrph'
          : walletType;
      const paymentMethodRes = await createPaymentMethod(
        walletTypeForApi,
        expirySeconds,
      );
      ...
      const paymentIntentRes = await createPaymentIntent(amount, [
        walletTypeForApi,
      ]);
      ...
      if (process.env.NEXT_PUBLIC_PAYMONGO_LIVE === 'true') {
        const qrImage = attachRes.data.attributes.next_action.code.image_url;
  ...
  return (
    <Box minHeight="100vh" bg="bg.canvas">
      <Container maxW="container.md" py={4}>
        <Stack gap={4}>
          ...
          <Flex
            ...
            pt={2}
          >
            <Button variant="outline" onClick={handleBack} flex={1} size="md">
              Cancel
            </Button>
            <Button
              colorPalette="green"
      ...
      <DialogRoot
        ...
        onOpenChange={(e) => !e.open && setIsWalletDialogOpen(false)}
        size="sm"
      >
        <DialogContent>
          <DialogHeader>
          ...
          <DialogBody>
            <Stack gap={2}>
              <Button
                ...
                onClick={() => {
                  if (process.env.NEXT_PUBLIC_PAYMONGO_LIVE === 'true') {
                    // In live environment, only QRPH is supported, so treat GCash selection as QRPH
                    handleWalletSelect('qrph');
                  } else {
                    handleWalletSelect('gcash');
                  }
                }}
                height="auto"
                py={4}
              ...
              <Button
                ...
                onClick={() => {
                  if (process.env.NEXT_PUBLIC_PAYMONGO_LIVE === 'true') {
                    // In live environment, only QRPH is supported, so treat PayMaya selection as QRPH
                    handleWalletSelect('qrph');
                  } else {
                    handleWalletSelect('paymaya');
                  }
                }}
                height="auto"
                py={4}
      ...
      <DialogRoot
        open={isQrDialogOpen}
        onOpenChange={(e) => !e.open && setIsQrDialogOpen(false)}
        size="md"
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Open your {selectedWallet === 'gcash' ? 'GCash' : 'PayMaya'} app
              ...
              {isProcessingPayment ? (
                ...
                <>
                  ...
                  <Stack gap={2} textAlign="center">
                    ...
                    <Text fontSize="xs" color="fg.muted">
                      Scan with your{' '}
                      {selectedWallet === 'gcash' ? 'GCash' : 'PayMaya'} app
                    </Text>
                    <Text fontSize="xs" color="fg.muted">
